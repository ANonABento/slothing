...
test.describe("Settings - Layout", () => {
  ...
  test("displays theme section", async ({ page }) => {
    ...
    if (await themeSection.first().isVisible({ timeout: 1000 }).catch(() => false)) {
  ...
  test("displays Google connection section", async ({ page }) => {
    ...
    if (await googleSection.first().isVisible({ timeout: 1000 }).catch(() => false)) {
...
test.describe("Settings - LLM Configuration", () => {
  ...
  test("shows provider selection", async ({ page }) => {
    const providerSelect = page.getByRole("combobox", { name: /provider/i }).or(
      page.getByRole("button", { name: /openai|anthropic|ollama|provider/i })
    );

    if (await providerSelect.first().isVisible({ timeout: 1000 }).catch(() => false)) {
      await expect(providerSelect.first()).toBeVisible();
    }
  });

  test("shows model selection", async ({ page }) => {
    const modelSelect = page.getByRole("combobox", { name: /model/i }).or(
      page.getByRole("button", { name: /gpt|claude|model/i })
    );

    if (await modelSelect.first().isVisible({ timeout: 1000 }).catch(() => false)) {
  ...
  test("shows API key input", async ({ page }) => {
    const apiKeyInput = page.getByLabel(/api key/i).or(
      page.getByPlaceholder(/api key|sk-/i)
    );

    if (await apiKeyInput.first().isVisible({ timeout: 1000 }).catch(() => false)) {
...
test.describe("Settings - Google Integration", () => {
  ...
  test("shows Google connection status", async ({ page }) => {
    const googleStatus = page.getByText(/google|connected|disconnected|connect/i);
    await expect(googleStatus.first()).toBeVisible();
  });

  test("has connect/disconnect button", async ({ page }) => {
    const connectButton = page.getByRole("button", { name: /connect|disconnect|google/i });
    if (await connectButton.first().isVisible({ timeout: 1000 }).catch(() => false)) {
...
test.describe("Settings - Data Management", () => {
  ...
  test("shows backup/export options", async ({ page }) => {
    ...
    if (await backupSection.first().isVisible({ timeout: 1000 }).catch(() => false)) {
  ...
  test("has dangerous zone with confirmation", async ({ page }) => {
    ...
    if (await dangerSection.first().isVisible({ timeout: 1000 }).catch(() => false)) {
      // Danger actions should require confirmation
      const deleteButton = page.getByRole("button", { name: /delete|reset/i });
      if (await deleteButton.first().isVisible({ timeout: 500 }).catch(() => false)) {
        await deleteButton.first().click();

        // Should show confirmation dialog
        const confirmation = page.getByRole("alertdialog").or(
