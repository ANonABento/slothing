...
`src/components/settings/bento-router-admin-section.test.tsx > saves task policies with
separate provider and model controls` failed once in CI and passes locally every time,
including five consecutive isolated runs. It renders, waits for async data, fires a change
and immediately asserts a derived value — a state update landing between those two steps
under CI load explains it. Unrelated to this work; `main` is green. Left alone rather than
"fixed" blind.
...
- **A Next route file may export ONLY route handlers.** Exporting a helper for a test to
  import fails the generated route-type check and therefore `next build` — with the
  message "Route ... does not match the required types of a Next.js Route". Neither
  `pnpm run type-check` nor `pnpm run test:run` catches it, because those route types are
  generated by the build itself. Put the helper in `lib/` and import it from both. This
  cost a red CI run.
- **`pnpm run build` is not one of the three gates** (type-check, test, lint), but the E2E
  jobs build the app, so a build break still fails CI — one step later and less legibly.
  Run it before pushing anything that touches a route file.
