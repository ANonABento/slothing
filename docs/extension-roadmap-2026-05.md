...
**Permission strategy (confirmed 2026-05-12).** Request `webNavigation`. If the user denies (Firefox lets users decline optional permissions; Chrome's mandatory-permission flow means a denial = uninstall), fall back to a **prompted multi-step flow**: on every page transition within an in-progress application, the in-page sidebar shows a non-blocking toast "Looks like step 2 of 4 — fill this page too?" with Yes/No. Yes triggers the same fill against the new DOM; No dismisses for the rest of the flow. Implement both paths.
...
- **#36 Workday — `webNavigation` permission:** ✅ request it. Fallback if user denies: prompted "fill this page too?" toast per step (see #36 plan above).
- **PR workflow:** every phase ships as PR(s) for review, no direct pushes to main. P1 = one bundled PR; P2/P3/P4 = one PR per task.

## Open questions (non-blocking, sensible defaults applied)

- **#38 LinkedIn passive — daily cap of 50** is hard-coded for now; revisit after dogfood.
- **#40 AI sidebar — LLM cost cap of 5 calls/day/user** via existing rate limiter; revisit.
