import { PROBLEMATIC_CHARACTERS } from "./ats-characters";
...
export function scoreAtsFriendliness(input: ResumeScoreInput): SubScore {
  ...
  if (longLines.length >= 4) {
    deductions += 2;
    notes.push(
      "Very long lines may indicate multi-column or table formatting.",
    );
  ...
  return {
    ...
    notes,
    evidence:
      evidence.length > 0 ? evidence : ["No ATS formatting issues detected."],
  };
}
