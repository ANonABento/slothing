...
export class WaterlooWorksScraper extends BaseScraper {
  ...
  async scrapeJobListing(): Promise<ScrapedJob | null> {
    ...
    try {
      await this.waitForElement(
        ".posting-details, .job-posting-details, #postingDiv",
        3000,
      );
    } catch {
  ...
  async scrapeJobList(): Promise<ScrapedJob[]> {
    ...
    if (this.isLoginPage()) {
      console.log(
        "[Columbus] Waterloo Works: Please log in to scrape job list",
      );
      return jobs;
    }
    ...
    const cards = document.querySelectorAll(
      '.job-card, .posting-card, [class*="posting-item"]',
    );
    for (const card of cards) {
      try {
        const titleEl = card.querySelector(
          ".job-title, .posting-title, h3, h4",
        );
        const companyEl = card.querySelector(
          '.employer-name, .company-name, [class*="employer"]',
        );
  ...
  private extractLocation(): string | null {
    const selectors = [".job-location", ".location", '[class*="location"]'];
  ...
  private extractDescription(): string | null {
    ...
    const mainContent = document.querySelector(
      ".posting-details, #postingDiv, .job-posting-details",
    );
    if (mainContent) {
