...
async function openAnswerBankSeed(question) {
    const auth = await sendMessage(Messages.getAuthStatus());
    const base = (auth.success && auth.data?.apiBaseUrl) || "http://localhost:3000";
        ...
        try {
            ...
            mountAnswerBankButton(textarea, {
                onMatch: async (q, limit) => {
                    const response = await sendMessage(Messages.matchAnswerBank({ q, limit: limit ?? 3 }));
                    if (!response.success) {
                ...
                onGenerate: (q) => {
                    ...
                    openAnswerBankSeed(q).catch((err) => console.warn("[Columbus] open bank failed:", err));
                },
            });
        }
        catch (err) {
