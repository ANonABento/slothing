...
test.skip(
  true,
  "Requires an authenticated NextAuth test fixture and analytics seed data.",
);
...
test.describe("Analytics - Layout", () => {
  ...
  test("displays page heading", async ({ page }) => {
    await expect(
      page.getByRole("heading", { name: /analytics|insights|metrics/i }),
    ).toBeVisible();
  });
  ...
  test("displays stats overview", async ({ page }) => {
    ...
    const statsCards = page.locator(
      ".card, [class*='Card'], .rounded-xl.border",
    );
  ...
  test("displays charts or visualizations", async ({ page }) => {
    ...
    const charts = page.locator(
      "canvas, svg[class*='chart'], [class*='Chart'], [class*='chart']",
    );
    if (
      await charts
        .first()
...
test.describe("Analytics - Data Display", () => {
  ...
  test("shows date range selector or timeframe", async ({ page }) => {
    const dateSelector = page
      .getByRole("combobox", { name: /date|time|range|period/i })
      .or(page.getByRole("button", { name: /week|month|year|all time/i }));
...
test.describe("Analytics - Export", () => {
  ...
  test("export options are clickable", async ({ page }) => {
    const exportButton = page
      .getByRole("button", { name: /export|sheets/i })
      .first();
    ...
    if (await exportButton.isVisible({ timeout: 1000 }).catch(() => false)) {
      ...
      const downloadStarted = await page
