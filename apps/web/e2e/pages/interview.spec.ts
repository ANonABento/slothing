...
test.describe("Interview - Layout", () => {
  ...
  test("displays job selection", async ({ page }) => {
    ...
    if (await jobSelect.first().isVisible({ timeout: 1000 }).catch(() => false)) {
  ...
  test("shows start button or practice area", async ({ page }) => {
    ...
    const practiceArea = page.locator("[class*='interview'], [class*='chat'], [class*='session']");

    const hasStart = await startButton.first().isVisible({ timeout: 1000 }).catch(() => false);
    const hasPractice = await practiceArea.first().isVisible({ timeout: 1000 }).catch(() => false);
...
test.describe("Interview - Session Start", () => {
  ...
  test("can start interview session", async ({ page }) => {
    ...
    if (await startButton.isVisible({ timeout: 1000 }).catch(() => false)) {
      ...
      const sessionUI = page.locator("[class*='question'], [class*='interview'], textarea");
      const dialog = page.getByRole("dialog");

      await page.waitForTimeout(500);

      const hasSession = await sessionUI.first().isVisible({ timeout: 2000 }).catch(() => false);
  ...
  test("shows question types or categories", async ({ page }) => {
    const categories = page.getByText(/behavioral|technical|situational|common/i);
...
test.describe("Interview - Text Mode", () => {
  ...
  test("text input is available for answers", async ({ page }) => {
    // Start session first
    const startButton = page.getByRole("button", { name: /start|begin|practice|text/i }).first();
    ...
    if (await answerInput.first().isVisible({ timeout: 2000 }).catch(() => false)) {
...
test.describe("Interview - Save to Docs", () => {
  ...
  test("Google Docs save option available", async ({ page }) => {
    ...
    const docsButton = page.getByRole("button", { name: /save.*docs|google.*docs|export/i });
...
test.describe("Interview - Past Sessions", () => {
  ...
  test("shows session history if available", async ({ page }) => {
    ...
    if (await historySection.first().isVisible({ timeout: 1000 }).catch(() => false)) {
