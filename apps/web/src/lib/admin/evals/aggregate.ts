  ...
  return {
    avgScoreGpt55: keys.map((key) => {
      ...
      return {
        ...
        value: bucket.gptCount ? round(bucket.gptTotal / bucket.gptCount, 2) : 0,
      };
    }),
    ...
    cost: keys.map((key) => {
      const bucket = buckets.get(key)!;
      return { date: key, label: trendLabel(key), value: round(bucket.cost, 4) };
    }),
    runs: keys.map((key) => {
      const bucket = buckets.get(key)!;
...
export function buildDashboardPayload(
  ...
  for (const report of sortedReports) {
    ...
    if (!setLabels.has(key)) {
      const size = new Set(report.results.map((result) => result.testCaseId)).size;
