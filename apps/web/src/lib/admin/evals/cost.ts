...
const TEST_CASE_BY_ID = new Map(
  TEST_CASES.map((testCase) => [testCase.id, testCase]),
);
...
export function estimateRunCostUsd(report: ComparisonReport): {
  ...
  for (const result of report.results) {
    const testCase =
      TEST_CASE_BY_ID.get(result.testCaseId) ??
      fallbackTestCase(report, result.testCaseId);
    ...
    judgeUsd += estimateModelCostUsd({
      ...
      inputChars: buildJudgePrompt(testCase, result.gpt55, result.claude)
        .length,
      outputChars:
