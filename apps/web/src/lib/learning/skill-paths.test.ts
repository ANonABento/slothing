...
describe("generateLearningPaths", () => {
  ...
  it("identifies skill gaps from job keywords", () => {
    ...
    const hasTypescript = result.paths.some((p) =>
      p.skill.toLowerCase().includes("typescript"),
    );
    const hasKubernetes = result.paths.some((p) =>
      p.skill.toLowerCase().includes("kubernetes"),
    );
