...
export function detectJobBoard(url: string): JobBoardType {
  ...
  if (
    ...
    lowercaseUrl.includes("boards.greenhouse")
  )
    return "greenhouse";
  if (lowercaseUrl.includes("lever.co") || lowercaseUrl.includes("jobs.lever"))
    return "lever";
...
export function detectJobType(
  ...
  if (
    lowercaseText.includes("intern") ||
    lowercaseText.includes("internship")
  ) {
    return "internship";
  }
  if (
    lowercaseText.includes("contract") ||
    lowercaseText.includes("contractor")
  ) {
    return "contract";
  }
  if (
    lowercaseText.includes("part-time") ||
    lowercaseText.includes("part time")
  ) {
    return "part-time";
  }
  if (
    lowercaseText.includes("full-time") ||
    lowercaseText.includes("full time")
  ) {
    return "full-time";
  }
...
export function extractSalary(text: string): string | undefined {
  ...
  const salaryPattern =
    /\$[\d,]+(?:k)?(?:\s*[-–]\s*\$[\d,]+(?:k)?)?(?:\s*(?:per|\/)\s*(?:year|yr|annum|annual))?/gi;
...
export function parseJobText(text: string, url?: string): ParsedJob {
  ...
  for (const line of lines.slice(0, 10)) {
    ...
    if (
      !title &&
      (trimmed.match(
        /^(senior|junior|lead|principal|staff|mid-level)?\s*(software|frontend|backend|full.?stack|devops|data|product|ux|ui)/i,
      ) ||
    ...
    if (
      !location &&
      (trimmed.match(
        /,\s*(ca|ny|tx|wa|fl|il|ma|co|ga|nc|pa|az|oh|va|nj|mi)/i,
      ) ||
...
export function parseJobJSON(data: Record<string, unknown>): ParsedJob {
  ...
  const reqField =
    data.requirements || data.qualifications || data.skills || [];
