...
function ResumeSectionView({ node, updateAttributes }: NodeViewProps) {
  ...
  return createElement(
    ...
    createElement(
      "h2",
      editableAttributeProps(updateAttributes, "title", "resume-section-title"),
      title,
    ),
...
function ContactInfoView({ node, updateAttributes }: NodeViewProps) {
  ...
  const detailNodes = details.flatMap(({ attribute, value }, index) => {
    ...
    return index === 0
      ? [field]
      : [createElement("span", { key: `${attribute}-separator` }, "|"), field];
  });
  ...
  return createElement(
    ...
    createElement("h1", editableAttributeProps(updateAttributes, "name"), name),
...
function getFirstTextSelectionPosition(section: {
...
export const ResumeSectionKeyboardNavigation = Extension.create({
  ...
  addKeyboardShortcuts() {
    const moveToAdjacentSection = (direction: 1 | -1) => () => {
  ...
  renderHTML({ HTMLAttributes, node }) {
    ...
    return [
      ...
      [
        ...
        ["div", {}, ["h3", {}, title], ["span", { class: "company" }, company]],
