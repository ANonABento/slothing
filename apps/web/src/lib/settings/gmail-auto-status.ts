...
export function getGmailAutoStatusEnabled(userId: string): boolean {
...
export function getGmailLastScannedAt(userId: string): string | null {
  return getSetting(GMAIL_LAST_SCANNED_AT_SETTING_KEY, userId);
}

export function setGmailLastScannedAt(iso: string, userId: string): void {
