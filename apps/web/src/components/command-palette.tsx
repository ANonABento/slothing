...
export function CommandPalette() {
  ...
  const grouped = useMemo(() => {
    ...
    if (recent.length > 0)
      groups.push({ category: "recent", label: "Recent", items: recent });
    if (navigate.length > 0)
      groups.push({ category: "navigate", label: "Navigate", items: navigate });
    if (actions.length > 0)
      groups.push({ category: "actions", label: "Actions", items: actions });
    ...
    (e: React.KeyboardEvent) => {
      switch (e.key) {
        ...
        case "ArrowUp":
          e.preventDefault();
          setSelectedIndex(
            (i) => (i - 1 + flatItems.length) % Math.max(flatItems.length, 1),
          );
          break;
        case "Enter":
          e.preventDefault();
              ...
              <div key={group.category}>
                ...
                {group.items.map((item) => {
                  ...
                  const Icon =
                    ...
                    (item.category === "recent" ? Clock : ArrowRight);
