...
vi.mock("@/lib/db/learned-answers-migration", () =>
  globalThis.__contractRouteMocks!.createContractModuleMock(
    "@/lib/db/learned-answers-migration",
  ),
);
...
describe("/api/answer-bank/migrate-personal-facts route contract", () => {
  beforeEach(() => {
    resetContractMocks();
  });
  ...
  it("invokes the real POST handler and returns an HTTP response contract", async () => {
    const response = await invokeRouteHandler(
      POST,
      jsonRequest(
        "http://localhost/api/answer-bank/migrate-personal-facts",
        representativeBody(),
        "POST",
