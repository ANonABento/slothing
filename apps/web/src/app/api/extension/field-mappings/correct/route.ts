...
import { ensureFieldMappingsCorrectionColumnsAsync } from "@/lib/db/field-mappings-schema";
...
export async function POST(request: NextRequest) {
  ...
  try {
    await ensureFieldMappingsCorrectionColumnsAsync();

    const userId = authResult.userId;
    const now = nowIso();

    const existingResult = await getClient().execute({
      sql: `SELECT id, hit_count FROM field_mappings
         WHERE user_id = ? AND domain = ? AND field_signature = ?
         LIMIT 1`,
      args: [userId, domain, fieldSignature],
    });
    ...
    if (existing) {
      ...
      await getClient().execute({
        sql: `UPDATE field_mappings
         SET observed_value = ?,
             field_type = ?,
             hit_count = ?,
             last_seen_at = ?
         WHERE id = ? AND user_id = ?`,
        args: [userValue, fieldType, nextHitCount, now, existing.id, userId],
      });
    ...
    await getClient().execute({
      sql: `INSERT INTO field_mappings
