...
describe("/api/profile route contract", () => {
  beforeEach(() => {
    resetContractMocks();
  });
  ...
  it("invokes the real GET handler and returns an HTTP response contract", async () => {
    const response = await invokeRouteHandler(
      GET,
      getRequest("http://localhost/api/profile", {
  ...
  it("invokes the real PUT handler and returns an HTTP response contract", async () => {
    const response = await invokeRouteHandler(
      PUT,
      jsonRequest("http://localhost/api/profile", representativeBody(), "PUT", {
        "x-extension-token": "test-token",
      }),
      routeContext(),
    );
  ...
  it("returns an HTTP error response for malformed mutation input", async () => {
    ...
    const response = await invokeRouteHandler(
      PUT,
      invalidJsonRequest("http://localhost/api/profile", "PUT"),
      routeContext(),
    );
