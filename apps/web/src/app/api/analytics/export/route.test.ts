...
vi.mock("@/lib/analytics/success-metrics", () =>
  globalThis.__contractRouteMocks!.createContractModuleMock(
    "@/lib/analytics/success-metrics",
  ),
);

vi.mock("@/lib/analytics/report-generator", () =>
  globalThis.__contractRouteMocks!.createContractModuleMock(
    "@/lib/analytics/report-generator",
  ),
);
...
describe("/api/analytics/export route contract", () => {
  beforeEach(() => {
    resetContractMocks();
  });
  ...
  it("invokes the real GET handler and returns an HTTP response contract", async () => {
    const response = await invokeRouteHandler(
      GET,
      getRequest("http://localhost/api/analytics/export", {
