...
describe("/api/import route contract", () => {
  beforeEach(() => {
    resetContractMocks();
  });
  ...
  it("invokes the real POST handler and returns an HTTP response contract", async () => {
    const response = await invokeRouteHandler(
      POST,
      jsonRequest("http://localhost/api/import", representativeBody(), "POST", {
  ...
  it("returns an HTTP error response for malformed mutation input", async () => {
    ...
    const response = await invokeRouteHandler(
      POST,
      invalidJsonRequest("http://localhost/api/import", "POST"),
      routeContext(),
    );
