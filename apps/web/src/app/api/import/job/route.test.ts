...
vi.mock("@/lib/import/job-parser", () =>
  globalThis.__contractRouteMocks!.createContractModuleMock(
    "@/lib/import/job-parser",
  ),
);
...
describe("/api/import/job route contract", () => {
  beforeEach(() => {
    resetContractMocks();
  });
  ...
  it("returns an HTTP error response for malformed mutation input", async () => {
    ...
    const response = await invokeRouteHandler(
      POST,
      invalidJsonRequest("http://localhost/api/import/job", "POST"),
      routeContext(),
    );
