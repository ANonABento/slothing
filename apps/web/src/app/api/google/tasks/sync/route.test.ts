...
describe("/api/google/tasks/sync route contract", () => {
  beforeEach(() => {
    resetContractMocks();
  });
  ...
  it("invokes the real GET handler and returns an HTTP response contract", async () => {
    const response = await invokeRouteHandler(
      GET,
      getRequest("http://localhost/api/google/tasks/sync", {
  ...
  it("invokes the real POST handler and returns an HTTP response contract", async () => {
    const response = await invokeRouteHandler(
      POST,
      jsonRequest(
        "http://localhost/api/google/tasks/sync",
        representativeBody(),
        "POST",
        { "x-extension-token": "test-token" },
      ),
  ...
  it("returns an HTTP error response for malformed mutation input", async () => {
    ...
    const response = await invokeRouteHandler(
      POST,
      invalidJsonRequest("http://localhost/api/google/tasks/sync", "POST"),
      routeContext(),
    );
