...
describe("benchmark dataset", () => {
  ...
  it("uses unique IDs", () => {
    ...
    expect(
      new Set(dataset.cases.map((benchmarkCase) => benchmarkCase.id)).size,
    ).toBe(250);
  });
  ...
  it("gives every resume exactly one job per scenario", () => {
    for (const resume of dataset.resumes) {
      const resumeJobs = dataset.jobs.filter(
        (job) => job.resumeId === resume.id,
      );
      expect(resumeJobs).toHaveLength(5);
      expect(new Set(resumeJobs.map((job) => job.scenario))).toEqual(
        new Set(scenarios),
      );
    }
  });
  ...
  it("keeps every case pair unique and scenario-aligned", () => {
    ...
    for (const benchmarkCase of dataset.cases) {
      ...
      pairs.add(pair);
      expect(jobsById.get(benchmarkCase.jdId)?.scenario).toBe(
  ...
  it("covers the required field and seniority diversity", () => {
    expect(
      new Set(dataset.resumes.map((resume) => resume.field)).size,
    ).toBeGreaterThanOrEqual(6);
    expect(
      new Set(dataset.resumes.map((resume) => resume.seniorityLevel)),
    ).toEqual(new Set(["new_grad", "junior", "mid", "senior", "principal"]));
  });

  it("keeps reference cases stratified and high quality", () => {
    const caseById = new Map(
      dataset.cases.map((benchmarkCase) => [benchmarkCase.id, benchmarkCase]),
    );
