 ...
 */
export function normalizeAnalysisResult(raw: LLMAnalysisResult): AnalyzedTemplate {
  return {
    styles: {
      ...
      accentColor: isValidHexColor(raw.accentColor) ? raw.accentColor! : "#333333",
      layout: isValidEnum(raw.layout, VALID_LAYOUTS) ? raw.layout as TemplateStyles["layout"] : "single-column",
      headerStyle: isValidEnum(raw.headerStyle, VALID_HEADER_STYLES) ? raw.headerStyle as TemplateStyles["headerStyle"] : "left",
      bulletStyle: isValidEnum(raw.bulletStyle, VALID_BULLET_STYLES) ? raw.bulletStyle as TemplateStyles["bulletStyle"] : "disc",
      sectionDivider: isValidEnum(raw.sectionDivider, VALID_SECTION_DIVIDERS) ? raw.sectionDivider as TemplateStyles["sectionDivider"] : "line",
    },
    charsPerLine: typeof raw.charsPerLine === "number" && raw.charsPerLine > 0 ? raw.charsPerLine : 80,
    margins: {
...
export function analyzeTemplateHeuristic(text: string): AnalyzedTemplate {
  ...
  const charsPerLine =
    contentLines.length > 0
      ? Math.round(contentLines.reduce((sum, l) => sum + l.length, 0) / contentLines.length)
      : 80;
  ...
  const headerStyle: TemplateStyles["headerStyle"] =
    firstRawLine.trim().length > 0 && lines.length > 1 && isCentered(firstRawLine, charsPerLine)
      ? "centered"
      : "left";
...
function isValidEnum<T extends string>(value: unknown, options: readonly T[]): value is T {
