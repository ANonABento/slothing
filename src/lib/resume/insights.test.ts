...
describe("Insight Generation", () => {
  ...
  describe("generateInsights", () => {
    it("should return at most 5 insights", () => {
      const data = makeData({
        ...
        snapshots: [
          makeSnapshot({ snapshotDate: "2024-01-08", profileCompleteness: 90, jobsApplied: 2, jobsInterviewing: 0 }),
          makeSnapshot({ snapshotDate: "2024-01-15", profileCompleteness: 70, jobsApplied: 2, jobsInterviewing: 0 }),
        ],
      });
  ...
  describe("analyzeMissingKeywords", () => {
    ...
    it("should not flag keywords already in profile", () => {
      const jobs = [makeJob({ keywords: ["javascript", "react", "typescript"] })];
      const data = makeData({ jobs });
      const insights = analyzeMissingKeywords(data);
  ...
  describe("analyzeAtsTrend", () => {
    ...
    it("should return empty with fewer than 2 snapshots", () => {
      const insights = analyzeAtsTrend(makeData({ snapshots: [makeSnapshot()] }));
      expect(insights).toEqual([]);
    });
  ...
  describe("analyzeResumePerformance", () => {
    it("should compare resume scores for interview vs non-interview jobs", () => {
      ...
      const insights = analyzeResumePerformance(makeData({ jobs, generatedResumes }));
    ...
    it("should return empty with fewer than 2 resumes", () => {
      const insights = analyzeResumePerformance(
        makeData({ generatedResumes: [{ id: "r1", jobId: "job-1", matchScore: 80, createdAt: "2024-01-10" }] })
      );
      expect(insights).toEqual([]);
    });
    ...
    it("should return insight when no numbers in experience", () => {
      const profile = makeProfile({
        experiences: [{
          ...
          highlights: ["Worked on frontend features", "Collaborated with team"],
          skills: ["React"],
        }],
      });
  ...
  describe("analyzeApplicationMomentum", () => {
    it("should encourage when momentum slowed", () => {
      const snapshots = [
        makeSnapshot({ snapshotDate: "2024-01-08", jobsApplied: 5, jobsInterviewing: 1 }),
        makeSnapshot({ snapshotDate: "2024-01-15", jobsApplied: 5, jobsInterviewing: 1 }),
      ];
    ...
    it("should celebrate when interviews increased", () => {
      const snapshots = [
        makeSnapshot({ snapshotDate: "2024-01-08", jobsApplied: 3, jobsInterviewing: 0 }),
        makeSnapshot({ snapshotDate: "2024-01-15", jobsApplied: 5, jobsInterviewing: 2 }),
      ];
