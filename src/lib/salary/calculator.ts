...
const BASE_SALARY_BY_EXPERIENCE: Record<string, { min: number; median: number; max: number }> = {
...
export function calculateTotalCompensation(offer: CompensationOffer): TotalCompensation {
  ...
  const signingBonusAnnualized = offer.signingBonus ? offer.signingBonus / 4 : 0; // Spread over 4 years

  const totalAnnual = annualBase + annualBonus + annualEquity + signingBonusAnnualized;
  const totalFourYear =
    annualBase * 4 +
    annualBonus * 4 +
    (offer.equityValue ?? 0) +
...
export function compareOffers(offers: CompensationOffer[]): {
  ranked: Array<{ offer: CompensationOffer; totalComp: TotalCompensation; rank: number }>;
  bestOverall: string;
  bestBase: string;
  bestEquity: string;
} {
  ...
  const bestBase =
    [...withTotals].sort((a, b) => b.totalComp.annualBase - a.totalComp.annualBase)[0]?.offer
      .company ?? "";
  const bestEquity =
    [...withTotals].sort((a, b) => b.totalComp.annualEquity - a.totalComp.annualEquity)[0]?.offer
      .company ?? "";
...
export function generateCounterOffer(
  ...
  const percentIncrease = ((suggestedBase - currentOffer.baseSalary) / currentOffer.baseSalary) * 100;
  ...
  return { suggestedBase, percentIncrease: Math.round(percentIncrease), reasoning };
}
