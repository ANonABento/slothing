...
describe("getErrorToastDescription", () => {
  it("uses fallback descriptions for generic unknown errors", () => {
    expect(getErrorToastDescription(42, "Please try again.")).toBe("Please try again.");
  });

  it("uses the first line of error messages", () => {
    expect(getErrorToastDescription(new Error("Request failed\nstack line"))).toBe("Request failed");
  });
