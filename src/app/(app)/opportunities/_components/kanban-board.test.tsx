...
describe("KanbanBoard", () => {
    ...
    render(
      <KanbanBoard
        opportunities={[
          ...
          { ...baseOpportunity, id: "opp-2", title: "Expired Role", status: "expired" },
          {
            ...baseOpportunity,
            id: "opp-3",
  ...
  it("prompts for a closed substate when dropping into Closed", () => {
    ...
    const card = screen.getByText("Member of Technical Staff").closest("article");
    const closedLane = screen.getByRole("region", {
      name: "Closed opportunities",
    });
  ...
  it("lists hidden lanes with cards and exposes one-click reveal", () => {
    ...
    render(
      <KanbanBoard
        opportunities={[
          baseOpportunity,
          { ...baseOpportunity, id: "opp-2", title: "Pending Role", status: "pending" },
        ]}
        visibleLanes={["saved"]}
        onStatusChange={vi.fn()}
