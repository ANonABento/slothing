...
import type { EvalCase, EvalMetric, GeneratorOutput, MetricScore } from "../types.js";
import { ACTION_VERBS } from "./action-verbs.js";

function outputText(output: GeneratorOutput): string {
  if (output.kind === "resume") {
    return output.resume ? resumeToKeywordSearchText(output.resume) : output.rawText;
  }
  return output.text;
}
...
export const keywordOverlapMetric: EvalMetric = (testCase, output) => {
  ...
  if (output.kind === "resume" && output.resume) {
    const analysis = analyzeResumeFit(testCase.jobDescription, output.resume, keywords);
    return {
  ...
  return {
    ...
    details: keywords.length > 0 ? { matched, missing } : { note: "no keywords" },
  };
};
...
export const actionVerbMetric: EvalMetric = (_testCase, output) => {
  ...
  const score = Math.min(1, matched.length / Math.max(3, Math.ceil(words / 120)));
  return {
    name: "action_verbs",
